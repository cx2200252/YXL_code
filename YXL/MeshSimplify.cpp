...
namespace MeshSimplify
{
	...
	std::vector<int> fix_vertex_indices;
	...
	{
		vertices.resize(cnt_v);
		memset(vertices.data(), 0, vertices.size() * sizeof(Vertex));
		for (int i = 0, idx=0; i < vertices.size(); i++)
		{
			vertices[i].p.x = v[idx++];
			vertices[i].p.y = v[idx++];
			vertices[i].p.z = v[idx++];
		} // i

		triangles.resize(cnt_tri);
		memset(triangles.data(), 0, triangles.size() * sizeof(Triangle));
		for (int i = 0,idx=0; i < triangles.size(); i++)
		{
			triangles[i].v[0] = tris[idx++];
			triangles[i].v[1] = tris[idx++];
			triangles[i].v[2] = tris[idx++];
		}
	}
	...
	{
		const int nVertices = get_result_vert_num();
		for (int i = 0, idx = 0; i < nVertices; i++)
		{
			v[idx++] = vertices[i].p.x;
			v[idx++] = vertices[i].p.y;
			v[idx++] = vertices[i].p.z;
		}

		const int nTris = get_result_tri_num();
		for (int i = 0, idx=0; i < nTris; i++)
		{
			tris[idx++] = triangles[i].v[0];
			tris[idx++] = triangles[i].v[1];
			tris[idx++] = triangles[i].v[2];
		}
	}

	bool IsFix(int vertex_idx)
	{
		auto& vertex = vertices[vertex_idx];

		loopk(0, vertex.tcount)
		{
			Triangle &t = triangles[refs[vertex.tstart + k].tid];
			loopj(0, 3)
			{
				for (auto idx : fix_vertex_indices)
					if (t.v[j] == idx)
						return true;
			}
		}
	...
	void process(const int target_tri_count, const int target_vert_count, std::vector<Float3>& fix_points, const int max_iter, double agressiveness, bool verbose)
	{
		if(fix_points.empty()==false)
			...
			{
				auto& v = vertices[i];
				for (auto pt : fix_points)
					if(pt.x==v.p.x && pt.y==v.p.y && pt.z==v.p.z)
						fix_vertex_indices.push_back(i);
			}
		// init
		...
		for (int iteration = 0; iteration < max_iter; iteration++)
		{
			...
			loopi(0, triangles.size())
			{
				...
				if (IsFix(t.v[0])|| IsFix(t.v[1]) || IsFix(t.v[2]))
				{
					//printf("fix \n");
					continue;
				}
				...
				loopj(0, 3)if (t.err[j] < threshold)
				{
					...
					/*int i = 322374;
						if (triangles[i].v[0] == triangles[i].v[1] || triangles[i].v[0] == triangles[i].v[2] || triangles[i].v[1] == triangles[i].v[2])
						{
							printf("abc\n");
						}*/
	...
	{
		//body
		std::vector<Vertex> tmpV[2];
		std::vector<Triangle> tmpTri[2];
		bool isBg[2] = {true, false};
		
		loopk(0,2)
		{
			double bg_depth = vertices[0].p.z;
			auto& _vertices = tmpV[k];
			auto& _tris = tmpTri[k];
			_vertices.reserve(vertices.size());
			_tris.reserve(triangles.size());
			...
			loopi(0, triangles.size())
				if (!triangles[i].deleted)
				{
					...
					bool is_bg = true;
					loopj(0, 3)
						if (abs(vertices[t.v[j]].p.z - bg_depth)>1e-1)
						{
							is_bg = false;
							break;
						}
		...
		vertices.resize(tmpV[0].size() + tmpV[1].size());
		triangles.resize(tmpTri[0].size() + tmpTri[1].size());

		vertices.assign(tmpV[0].begin(), tmpV[0].end());
		triangles.assign(tmpTri[0].begin(), tmpTri[0].end());

		const int ofs = vertices.size();
		vertices.insert(vertices.end(), tmpV[1].begin(), tmpV[1].end());
		for (auto tri : tmpTri[1])
		{
			loopi(0, 3)
				tri.v[i] += ofs;
